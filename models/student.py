...
class SchoolStudent(models.Model):
    ...
    class_id = fields.Many2one('school.class', string='Class / Section', domain="[('grade_id', '=', grade_id)]")
    ...
    attendance_percentage = fields.Float(string='Attendance %', compute='_compute_student_stats')
    total_fee_due = fields.Float(string='Fee Balance Due', compute='_compute_student_stats')
    books_borrowed_count = fields.Integer(string='Books Borrowed', compute='_compute_student_stats')
    exam_results_count = fields.Integer(string='Exam Results', compute='_compute_student_stats')

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('student_id_code', 'New') == 'New':
                vals['student_id_code'] = self.env['ir.sequence'].next_by_code('school.student') or 'STU/0001'
    ...
    def _compute_student_stats(self):
        for student in self:
            # Attendance
            att_lines = self.env['school.attendance.line'].search([('student_id', '=', student.id)])
            if att_lines:
                present = len(att_lines.filtered(lambda l: l.state in ['present', 'late']))
                student.attendance_percentage = (present / len(att_lines)) * 100.0
            else:
                student.attendance_percentage = 100.0

            # Fee Due
            fees = self.env['school.student.fee'].search([('student_id', '=', student.id)])
            student.total_fee_due = sum(f.amount_due for f in fees)

            # Books Borrowed
            issues = self.env['school.library.issue'].search([('student_id', '=', student.id), ('state', '=', 'issued')])
            student.books_borrowed_count = len(issues)

            # Exam Results
            results = self.env['school.exam.result'].search([('student_id', '=', student.id)])
